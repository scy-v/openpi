...
class Inference:
    def __init__(self, config_path: Path):
        ...
        self.infer_fps = model["infer_fps"]
        ...
        self.visualize = video.get("visualize", False)
    ...
    # --------------------------- ACTION EXECUTION --------------------------- #
    def execute_action(self, action: np.ndarray, block: bool = False):
        ...
        if block:
            ...
            self.rtde_c.moveJ(action, self._velocity, self._acceleration)
    ...
    def run(self):
        ...
        self.execute_action(self.initial_pose, block=True) # move to initial pose
        ...
        logging.info("Warming Up Inference")
        start = time.time()
        policy.infer(obs)
        logging.info(f"Warming Up Done, took {time.time() - start:.2f}s\n")
        # queue for passing latest inference result to action thread (size 1 keeps only latest)
        self._result_queue = queue.Queue(maxsize=1)
        # stop event for graceful shutdown
        self._stop_event = threading.Event()

        # threads
        self._infer_thread = threading.Thread(target=self._inference_loop, args=(policy,), daemon=True)
        self._action_thread = threading.Thread(target=self._actions_loop, daemon=True)          

        logging.info("========== Starting Inference and Action Threads ==========")
        # start threads
        self._infer_thread.start()
        self._action_thread.start()

        try:
            # wait until threads stop (KeyboardInterrupt)
            while not self._stop_event.is_set():
                time.sleep(0.1)
        except KeyboardInterrupt:
            logging.info("[INFO] KeyboardInterrupt detected. Stopping threads and saving recorded videos before exiting...")
    ...
    def _inference_loop(self, policy):
        """Run get_obs_state + policy.infer at self.infer_fps and publish latest result to queue."""
        infer_period = 1.0 / float(self.infer_fps) if self.infer_fps > 0 else 0.0
        infer_count = 1
        logging.info(f"[THREAD] Inference loop running at {self.infer_fps} FPS (period={infer_period}s)")
        fps_counter = FpsCounter("Inference")
        try:
            while not self._stop_event.is_set():
                ...
                try:
                    ...
                    logging.error(f"[ERROR] policy.infer failed: {e}")
                    result = None
                    ...
                    try:
                        if self._result_queue.full():
                            try:
                                ...
                                pass
                        self._result_queue.put_nowait((result, obs, infer_count))
                        infer_count += 1
                    except Exception as e:
                        logging.error(f"[ERROR] Failed to enqueue inference result: {e}")

                    # submit observation to recorder (record latest obs)
                    try:
                        self.recorder.submit_obs(obs)
                    except Exception as e:
                        logging.error(f"[ERROR] recorder.submit_obs failed: {e}")

                # sleep to maintain target fps
                elapsed = time.perf_counter() - start
                to_sleep = infer_period - elapsed
                if to_sleep > 0:
                    time.sleep(to_sleep)
                fps_counter.update()
    ...
    def _actions_loop(self):
        """Execute current action sequence step-by-step at self.action_fps."""
        action_period = 1.0 / float(self.action_fps) if self.action_fps > 0 else 0.0
        logging.info(f"[THREAD] Actions loop running at {self.action_fps} FPS (period={action_period:.4f}s)")
        fps_counter = FpsCounter("Action")
        ...
        try:
            while not self._stop_event.is_set():
                ...
                # If current sequence is empty or finished, try to get the latest inference result
                if current_actions is None or current_idx >= len(current_actions):
                    try:
                        result, obs, infer_idx = self._result_queue.get()
                        if self._is_first_action:
                            current_actions = result["actions"][:self.action_horizon]
                            self._is_first_action = False
                        else:
                            current_actions = result["actions"][self.action_horizon: 2*self.action_horizon]
                        current_idx = 0
                        logging.info(f"[ACTION] New action sequence received (len={len(current_actions)})")

                        # record actions metadata
                        try:
                            self.recorder.submit_actions(current_actions[:self.action_horizon], infer_idx, obs["prompt"])
                        except Exception as e:
                            logging.error(f"[ERROR] recorder.submit_actions failed: {e}")
                    except queue.Empty:
                        pass

                # Execute current action step
                if current_actions is not None and current_idx < len(current_actions):
                    try:
                        self.execute_action(current_actions[current_idx])
                        current_idx += 1
                    except Exception as e:
                        logging.error(f"[ERROR] execute_action failed: {e}")

                # Control action execution rate
                elapsed = time.perf_counter() - start
                to_sleep = action_period - elapsed
                if to_sleep > 0:
                    time.sleep(to_sleep)
                fps_counter.update()

        except Exception as e:
            logging.error(f"[ERROR] Actions loop encountered an error: {e}")
